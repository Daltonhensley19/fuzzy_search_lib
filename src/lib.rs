...
mod tests {
    ...
    fn it_works() {
        ...
        let reference_strs = load_corpus(corpus_path).unwrap();

        // Find closest string from the corpus
        let closest_str = find_closest_str("Dog", reference_strs.as_slice());

        println!("Closest string: {}", closest_str);
    }
    ...
    pub fn load_corpus<P: AsRef<Path>>(path: P) -> Result<Vec<String>, FuzzySearchError> {
        // Open corpus file
        let mut corpus_file = std::fs::File::open(path).map_err(|_| FuzzySearchError::UnableToOpenCorpusFile)?;
    ...
    pub fn find_closest_str<'a>(arg: &'a str, reference_strs: &'a [String]) -> String {
